 ...
 * file		jt_familytree model - jt_familytree.php
 *
 * @version	1.4.0
...
class JoaktreeModelJt_familytree extends JModelList {
	...
	private function _buildQuery() {
		...
		$query->select('lan.title AS language_title');
		$query->leftJoin(' #__languages       lan '
						.' ON ( CONVERT(lan.lang_code USING ascii) = CONVERT(jte.language USING ascii) ) '
						);
	...
	private function _buildContentWhere() {
		...
		if ( $filter_state ) {
			if ( $filter_state == 'P' ) {
				$wheres[] = 'jte.published = 1';
			} else if ($filter_state == 'U' ) {
				$wheres[] = 'jte.published = 0';
			}
		}
		...
		if ( $filter_language ) {
			$wheres[] = 'jte.language = ' . $this->_db->Quote($filter_language);
		}
	...
	public function getAction() {
		$tmp = JRequest::getVar('action');
		return ($tmp == 'assign') ? $tmp : null;
	}

	public function getTreeId() {
		$tmp = JRequest::getInt('treeId');
	...
	**         treedef_2 	- Finished setting up default trees 2 (1 language tree per person)
	**         treedef_3 	- Finished setting up default trees 3 (1 father/language tree per person)
	**         treedef_4 	- Finished setting up default trees 4 (1 mother/language tree per person)
	**         treedef_5 	- Finished setting up default trees 5 (1 partner/language tree per person)
	**         treedef_6 	- Finished setting up default trees 6 (lowest language tree)
