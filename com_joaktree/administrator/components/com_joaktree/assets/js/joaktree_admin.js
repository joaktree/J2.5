...
function HandleResponseGedcom(type, response) {
	...
	if ((r) && (r.status)) {
		...
		if (r.status == 'stop') {			
			...
			document.getElementById('head_finished').style.display = 'inline';
		}

		if (r.status == 'error') {
			document.getElementById('head_process').style.display  = 'none';
			document.getElementById('head_error').style.display    = 'inline';
		}
		...
		if (r.status != 'stop') {
			...
			if (r.persons > 0) {
				document.getElementById('l_persons_' + r.id).style.display = 'inline';
				document.getElementById('persons_' + r.id).value = r.persons;
			}
			
			if (r.families > 0) {
				document.getElementById('l_families_' + r.id).style.display = 'inline';
				document.getElementById('families_' + r.id).value = r.families;
			}

			if (r.sources > 0) {
				document.getElementById('l_sources_' + r.id).style.display = 'inline';
				document.getElementById('sources_' + r.id).value = r.sources;
			}

			if (r.repos > 0) {
				document.getElementById('l_repos_' + r.id).style.display = 'inline';
				document.getElementById('repos_' + r.id).value = r.repos;
			}

			if (r.notes > 0) {
				document.getElementById('l_notes_' + r.id).style.display = 'inline';
				document.getElementById('notes_' + r.id).value = r.notes;
			}

			if (r.unknown > 0) {
				document.getElementById('l_unknown_' + r.id).style.display = 'inline';
...
function assignFT(url1) {
	...
	if (!url1) { url1 = 'index.php?option=com_joaktree&view=jt_familytree&format=raw&tmpl=component&init=0'; }
