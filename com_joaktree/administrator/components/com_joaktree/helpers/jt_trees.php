...
//				$msg   = $this->_db->query( );
		 ...
		 case 'endtree':
			...
			$returnStatus = 'treedef_1';
		 	break;
		 case 'treedef_1':
			// Deal with mutiple language sites
			// find the default language of the site
			$langPar = JComponentHelper::getParams('com_languages');
			$lang    = $langPar->get('site', 'en-GB');	
			...
			$quer1 = 'UPDATE     #__joaktree_admin_persons  jan '
					...
					.'                                AND jte.language = '.$this->_db->quote($lang).' '
					.'                                ) '     
					.'                          WHERE      jtp2.app_id = '.$app_id.' '
					.'                          GROUP BY   jtp2.person_id '
					.'                          HAVING     COUNT(jtp2.tree_id) = 1 '
		 ...
		 case 'treedef_5':
			// PERSONS WITH MORE THAN 1 TREE
			// select all persons in the system without default tree
			// update with first tree (lowest number) of default language		
			$langPar = JComponentHelper::getParams('com_languages');
			$lang    = $langPar->get('site', 'en-GB');	
