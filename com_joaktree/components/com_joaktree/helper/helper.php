...
class JoaktreeHelper {
	...
	function getUserAccessLevels() {
 	...
 	function getTreeId($intern = false, $requestonly = false) {
		...
		if (!isset($_treeId)) {
			...
			$tmp1 = JRequest::getVar('treeId');
			$tmp2 = JRequest::getInt('treeId');
	...
	function getPersonId($intern = false, $requestonly = false) {
		...
		if (!isset($_personId)) {
 			$tmp = JRequest::getString('personId');
		...
		if (!isset($_appId)) {
			...
			$tmp = JRequest::getString('personId');	
 			...
 			if (!empty($tmp)) {
				...
				$tmp1 = JRequest::getVar('appId');
				$tmp  = JRequest::getInt('appId');
	...
	function getRelationId() {
		...
		if (!isset($_relationId)) {
			$tmp = JRequest::getString('relationId');
	...
	function getSourceId($optional = false) {
		...
		if (!isset($_sourceId)) {
			$tmp = JRequest::getString('sourceId');
	...
	function getTechnology() {
		...
		if (!isset($_technology)) {
			...
			if ($indCookie) {
				$tmp	= JRequest::getVar('tech', null, 'COOKIE', 1);
			}
			
			if (!isset($tmp)) {
				// if not found in cookie -> look in url
				$tmp	= JRequest::getVar('tech', null, 'GET');
			}
